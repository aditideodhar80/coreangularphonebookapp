...
export class PersonServiceProxy {
     ...
     * @return Success
     */
    getGetPersonForEdit(id: number | null | undefined): Observable<GetPersonForEditOutput> {
        let url_ = this.baseUrl + "/api/services/app/Person/GetGetPersonForEdit?";
        if (id !== undefined)
            url_ += "id=" + encodeURIComponent("" + id) + "&"; 
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetGetPersonForEdit(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetGetPersonForEdit(<any>response_);
                } catch (e) {
                    return <Observable<GetPersonForEditOutput>><any>_observableThrow(e);
                }
            } else
    ...
    protected processGetGetPersonForEdit(response: HttpResponseBase): Observable<GetPersonForEditOutput> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
